...
const MAX_STORED = 10;
const CACHE_FILE = path.join(process.cwd(), ".cache", "spotify-releases.json");
...
export async function getLatestReleases(
  ...
  inflight = (async () => {
    try {
      ...
      if (status === 429) {
        throw apiError(
          "Spotify rate limit reached. Wait a few minutes, then refresh. Cached data will load automatically once available.",
          429,
        );
      }
      throw err;
